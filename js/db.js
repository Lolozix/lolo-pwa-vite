...
async function createDB() {
    try {
        db = await openDB('banco', 1, {
            upgrade(db, oldVerion, newVersion, transaction) {
                switch (oldVersion) {
                    case 0:
                    case 1:
                        ...
                        // Criando um indice id na store, deve estar contido no objedo do banco.
                        store.createIndex('id', 'id');
                        showResult("Bqnco de dados criado com sucesso 😊");
                }
            }
        });
        showResult("Banco de dados aberto!");
    } catch (e) {
        showResult("Erro ao criar o banco de dados ☹️: " + e.message)
    }
}
